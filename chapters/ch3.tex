...
\par In this chapter, we give an in-depth review of formant trajectory extraction. As reviewed in section \ref{birdsong_review}, birdsong can be compared to human vowels in structure. Moreover, in subsection \ref{subsection_fmnts} we learnt that formant trajectories can characterise birdsong recordings.  
...
E(z) &= \mathcal{Z}\{\V{s}\} - \mathcal{Z}\left\{ \left(\sum_{i=1}^pa_ix_{n-i}\right)_n\right\} \\
...
\end{align*}
Recall that, given a signal $\V{x}$, we can find its linear prediction coefficients $\V{a}$ by using the Levinson-Durbin recursion to solve the associated linear system of equations $\V{r} = \V{Ra}$, where $\V{r}$ is the autocorrelation vector and $\V{R}$ is the (Toeplitz) autocorrelation matrix. We are now ready to present formants:
...
To conclude, in this section we presented the Linear Prediction of Speech framework and its relationship with birdsong production. To do so, we reviewed key concepts from the Digital Signal Processing literature. Formant trajectories were shown to characterise birdsong recordings when applied to a system that follows the source-filter model of speech production, like birdsong does. However, some issues still remain, particularly: is formant trajectory sufficient to fully characterise each bird species? How are trajectories of different length to be handled? How about stretched or shifted versions of the same motifs? 
\par This motivates the search for a more abstract model that encourages the learning of formant trajectories as symbols invariant to relatively small perturbations. These models should summarise the content and the sequence of the trajectories. As a result, methods to achieve this are presented in chapter \ref{chapter_hmms}. 
